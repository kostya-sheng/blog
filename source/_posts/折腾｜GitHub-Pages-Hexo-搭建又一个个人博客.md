...
如果你想了解如何安装 Hexo，可以查看 Hexo 的 [官方文档](https://hexo.io/docs/)。如果你也决定使用 NexT 主题，可以查看 NexT 的文档的「[Getting Started](https://theme-next.js.org/docs/getting-started/)」部分，它也会教你如何在当前系统上安装一个 Hexo 示例并最终预览运行在本地的博客网页。
...
1. 所见即所得
   在使用 Typora 时，输入的 Markdown 代码可以直接转化为渲染引擎效果。并且 Typora 支持使用 Safari 的开发者模式进行直接的 CSS 调试，我们可以通过这种方式自定义写文章时的视觉体验。相对于 Visual Studio Code 来说，我觉得它更像是一个纯粹的写作工具。
...
当然，我也反反复复查阅了 Hexo 及其 Next 主题的官方文档。但综合来看，在 AI 时代，有一点非常好：当你向 AI 描述一个问题时，它几乎可以帮你准确定位问题的根源，并提供非常清晰、可行的解决方案，一步步带着你解决掉。甚至最近很火的 Agents，已经可以直接代替你来执行这些操作了。
